...
defaultArgs = struct('GeometricContextArea', 0, 'GeometricContextGround', [], 'GeometricContextWalls', [], 'GeometricContextSky', [], ...
...
if args.GeometricContextArea
    [m,mind] = max(cat(3, args.GeometricContextGround, args.GeometricContextWalls, args.GeometricContextSky), [], 3);
    
    nbPixels = size(img,1)*size(img,2);
    visibilityFeatures.GeometricContextArea = cat(2, nnz(mind==1)./nbPixels, nnz(mind==2)./nbPixels, nnz(mind==3)./nbPixels);
...
if args.LogHistogram
    ...
    imgVecLogNorm = imgVecLogNorm - xrepmat(meanImgLog, size(imgVec, 1), 1);
    imgVecLogNorm = imgVecLogNorm ./ xrepmat(var(imgVecLogNorm, 1), size(imgVec, 1), 1);
